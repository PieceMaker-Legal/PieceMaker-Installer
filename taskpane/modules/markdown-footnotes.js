...
// taskpane/modules/markdown-footnotes-source.mjs
var FOOTNOTE_TOKEN_PATTERN = /\uE000PMFN:(\d+)\uE001/g;
function footnoteToken(index2) {
  return `\uE000PMFN:${index2}\uE001`;
}
function walk(node2, visitor, insideDefinition = false) {
  const nowInsideDefinition = insideDefinition || node2.type === "footnoteDefinition";
  visitor(node2, nowInsideDefinition);
  if (!Array.isArray(node2.children)) return;
  for (const child of node2.children) walk(child, visitor, nowInsideDefinition);
}
function serializeDefinitionBlocks(definition3) {
  return definition3.children.map((child) => toMarkdown(
...
function normalizeLegacyInlineFootnotes(markdown) {
  ...
  const normalized = markdown.replace(
    ...
    (_match, _legacyLabel, text4) => {
      ...
      definitions.push(`[^${identifier}]: ${text4.trim()}`);
      return `[^${identifier}]`;
    }
  );
...
function isEscaped(source, offset) {
  let slashes = 0;
  for (let index2 = offset - 1; index2 >= 0 && source[index2] === "\\"; index2--) slashes += 1;
  return slashes % 2 === 1;
}
function prepareMarkdownFootnotes(value, { acceptLegacy = true } = {}) {
  const original = String(value ?? "");
  const legacy = acceptLegacy ? normalizeLegacyInlineFootnotes(original) : { markdown: original, legacySyntaxUsed: false };
  ...
  walk(tree, (node2, insideDefinition) => {
    if (node2.type === "footnoteDefinition") {
      const identifier = node2.identifier;
      const blocks = serializeDefinitionBlocks(node2);
      const previous2 = definitions.get(identifier);
      if (previous2 && JSON.stringify(previous2.blocks) !== JSON.stringify(blocks)) {
        throw new Error(`D\xE9finitions contradictoires pour la note [^${node2.label || identifier}].`);
      }
      if (!previous2) definitions.set(identifier, { identifier, label: node2.label || identifier, blocks });
      definitionNodes.push(node2);
      return;
    }
    if (node2.type === "footnoteReference") {
      if (insideDefinition) {
        nestedReference = node2.label || node2.identifier;
      } else {
        references.push(node2);
      }
      return;
    }
    if (node2.type === "text" && !insideDefinition) {
  ...
  if (nestedReference) {
    throw new Error(`Les notes imbriqu\xE9es ne sont pas prises en charge : [^${nestedReference}].`);
  }
  if (unresolved.length > 0) {
    const labels = [...new Set(unresolved)].map((id) => `[^${id}]`).join(", ");
    throw new Error(`D\xE9finition de note manquante : ${labels}.`);
  }
  const usedIdentifiers = new Set(references.map((node2) => node2.identifier));
  const unused = [...definitions.values()].filter((definition3) => !usedIdentifiers.has(definition3.identifier));
  if (unused.length > 0) {
    throw new Error(`D\xE9finition de note sans r\xE9f\xE9rence : [^${unused[0].label}].`);
  }
  const edits = [];
  const footnotes = [];
  for (const node2 of references) {
    const definition3 = definitions.get(node2.identifier);
    if (!definition3) {
      throw new Error(`D\xE9finition de note manquante : [^${node2.label || node2.identifier}].`);
    }
    const start = node2.position?.start?.offset;
    const end = node2.position?.end?.offset;
    if (!Number.isInteger(start) || !Number.isInteger(end)) {
      throw new Error(`Position Markdown introuvable pour la note [^${definition3.label}].`);
    }
    const occurrence = footnotes.length;
    footnotes.push({ ...definition3 });
    edits.push({ start, end, replacement: footnoteToken(occurrence) });
  }
  for (const node2 of definitionNodes) {
    const start = node2.position?.start?.offset;
    const end = node2.position?.end?.offset;
    if (Number.isInteger(start) && Number.isInteger(end)) {
