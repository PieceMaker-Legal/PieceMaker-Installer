...
function managedAbsolutePath(repoRoot, relativePath) {
  const normalized = normalizeManagedPath(relativePath);
  if (!managedFileKind(normalized)) throw new Error('Ce fichier ne fait pas partie des fichiers administrables.');
...
function listManagedFiles(repoRoot) {
  ...
  return candidates.map((relativePath) => {
    const { absolute, normalized } = managedAbsolutePath(repoRoot, relativePath);
    return {
      path: normalized,
      ...
      kind: managedFileKind(normalized),
      exists: fs.existsSync(absolute),
    };
  });
}

function readManagedFile(repoRoot, relativePath) {
  const { absolute, normalized } = managedAbsolutePath(repoRoot, relativePath);
...
function createAdminRouter({
  ...
  router.get('/status', (req, res) => {
    ...
    const files = listManagedFiles(repoRoot);
    res.json({
      ok: true,
  ...
  router.get('/files', (req, res) => {
    res.json({ files: listManagedFiles(repoRoot) });
  });

  router.get('/file', (req, res) => {
    try {
      res.json(readManagedFile(repoRoot, req.query.path));
    } catch (error) {
      res.status(400).json({ error: error.message });
    }
  });
