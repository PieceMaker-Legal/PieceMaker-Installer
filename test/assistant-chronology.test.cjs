...
const { legalGraphPaths } = require('../websocket-server/legal-graph.cjs');
...
test('la chronologie assistant est triée, pseudonymisée et reprend les corrections structurées', async (t) => {
  ...
  assert.equal(chronology.source, 'piecemaker-legal-graph');
  ...
  const persistedGraph = JSON.parse(fs.readFileSync(legalGraphPaths(data.caseRoot).graph, 'utf8'));
  assert.doesNotMatch(JSON.stringify(persistedGraph), /Bernard Gilly|Société du Parc/);
