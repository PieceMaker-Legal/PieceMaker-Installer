...
test('readDocumentIndex tolerates a missing or corrupt file', () => {
  ...
  assert.deepEqual(readDocumentIndex(caseRoot), {
    version: 2, documents: {}, overrides: {}, entityDecisions: {}, revisions: [],
  });
...
test('overrides apply independently from entity label deanonymization', async () => {
  const caseRoot = fixture({ index: SAMPLE_INDEX(), mapping: SAMPLE_MAPPING() });
  writeDocumentIndexOverride(caseRoot, 'Courrier.pdf', {
    juridiction: 'Étude de Maître X',
  ...
  assert.equal(courrier.edited, true);
  assert.equal(courrier.juridiction, 'Étude de Maître X');
  assert.deepEqual(courrier.fields, [{ label: 'Note', value: 'texte libre en clair' }]);
  assert.equal(courrier.metadata.juridiction.source, 'admin_manual');
  assert.ok(courrier.qualityFlags.some((flag) => flag.field === 'juridiction'));
  fs.rmSync(caseRoot, { recursive: true, force: true });
});
